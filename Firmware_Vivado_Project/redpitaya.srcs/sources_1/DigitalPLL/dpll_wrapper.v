...
wire [8-1:0] monitor_I;
wire [8-1:0] monitor_Q;
...
wire bUseExternalLO;
wire [16-1:0] IQ_monitor_gain;

wire [16-1:0] external_LO_cosine;
wire [16-1:0] external_LO_sine;
   ...
   parallel_bus_register_64_bits_or_less # (
    ...
    parallel_bus_register_ddc_filter_select (
         ...
         .bus_address(cmd_addr), 
         .bus_data({cmd_data2in, cmd_data1in}), 
         .register_output({bUseExternalLO, select_phase_or_freq1, select_phase_or_freq0, ddc1_filter_select, ddc0_filter_select}), 
         .update_flag()
         );
   ...
   parallel_bus_register_32bits_or_less # (
    ...
    parallel_bus_register_32bits_or_less # (
        .REGISTER_SIZE(16),
        .REGISTER_DEFAULT_VALUE(32'd255),
        .ADDRESS(16'h8005)
    )
    parallel_bus_register_IQ_monitor_gain (
         .clk(clk1), 
         .bus_strobe(cmd_trig), 
         .bus_address(cmd_addr), 
         .bus_data({cmd_data2in, cmd_data1in}), 
         .register_output(IQ_monitor_gain), 
         .update_flag()
         );
    ...
    .data_input(ADCraw0),     // 
    .external_LO_cosine(external_LO_cosine),
    .external_LO_sine(external_LO_sine),
     
     // Configuration
    .boxcar_filter_size(boxcar_filter_size),
    .reference_frequency(reference_frequency0), 
    .ddc_filter_select(ddc0_filter_select),
    .bUseExternalLO(bUseExternalLO),
    .IQ_monitor_gain(IQ_monitor_gain),
...
//     );
    ...
    // The new DDC which generates a reference tone for the other input channel:
    wire [25-1:0] ddc1_k;
    ddc_external_ref_generator ddc_external_ref_generator_inst (
        .rst(                 rst_frontend1),
        .clk(                 clk1),
        .data_input(          ADCraw1),
        .reference_frequency( reference_frequency1),
        .ddc_filter_select(   ddc1_filter_select),
        .k(                   ddc1_k),
        .ref_cosine_out(      external_LO_cosine),
        .ref_sine_out(        external_LO_sine)
    );
    // for testing purposes
    reg [16-1:0] external_LO_cosine_reg;
    always @(posedge clk1) begin
        external_LO_cosine_reg <= external_LO_cosine;
    end
    //assign DACout1 = external_LO_cosine_reg;   

// Register which controls the frequency ratio generated by DDC1
parallel_bus_register_32bits_or_less # (
    .REGISTER_SIZE(25),
    .REGISTER_DEFAULT_VALUE(25'd262144),   // 1*2^18
    .ADDRESS(16'h9002)
)
parallel_bus_register_freq_ratio_ddc1  (
 .clk                           (clk1                       ), 
 .bus_strobe                    (cmd_trig                   ), 
 .bus_address                   (cmd_addr                   ), 
 .bus_data                      ({cmd_data2in, cmd_data1in} ), 
 .register_output               (ddc1_k                     ), 
 .update_flag                   (                           )
);
...
// Configurable target lock point for the cascade lock (when using pll0_output as the error signal for pll1)
wire signed [16-1:0] pll0_cascade_lock_point;
...
parallel_bus_register_cascade_lockpoint  (
 ...
 .bus_address                   (cmd_addr                   ), 
 .bus_data                      ({cmd_data2in, cmd_data1in} ), 
 .register_output               (pll0_cascade_lock_point    ), 
 .update_flag                   (                           )
);
...
multiplexer_3to1_async loop_filters_1_mux (
 ...
 .in2_mux                           (pll0_cascade_lock_error_signal),
 .out_mux                           (inst_frequency1            )
);

    // Add the offset and implement proper right_shifting with saturation to fit the 16-bits error signal into the 10 bits input
    cascade_lock_error_formatter cascade_lock_error_formatter_inst (
        .clk(clk1),
        .dac0_in(DACout0),
        .dac0_lockpoint_in(pll0_cascade_lock_point),
        .error1_out(pll0_cascade_lock_error_signal)
    );
...
wire [16-1:0] DACout1_normal_mode;
...
(  
    ...
    .data_output      (  DACout1_normal_mode),//DACout1                    ),  --- For debugging!
...
wire bCheckExtRefDemod;
parallel_bus_register_32bits_or_less # (
    .REGISTER_SIZE(1),
    .REGISTER_DEFAULT_VALUE(0),
    .ADDRESS(16'h6005)
)
parallel_bus_register_bCheckExtRefDemod (
    .clk(clk1), 
    .bus_strobe(cmd_trig), 
    .bus_address(cmd_addr), 
    .bus_data({cmd_data2in, cmd_data1in}), 
    .register_output(bCheckExtRefDemod), 
    .update_flag()
    );


    // mux the signal that we send to DAC1 between the demodulated reference cosine of DDC1, or the normal operation signal coming out of output_summing_dac1:
    reg [16-1:0] DACout1_reg;
    always @(posedge clk1) begin
        DACout1_reg <= (bCheckExtRefDemod == 1'b1 ? external_LO_cosine_reg : DACout1_normal_mode);
    end
    assign DACout1 = DACout1_reg;
...
assign status_flags[32-1:12] = 21'd0;
