...
class FreqErrorWindowWithTempControlV3(QtGui.QWidget):
    ...
    def __init__(self, sl, strTitle, sp, output_number=0, strNameTemplate='', custom_style_sheet='', port_number=0, xem_gui_mainwindow=0,rio_port=0, settings_window = 0):
        ...
        if settings_window:
            self.settings_window = weakref.proxy(settings_window)
        else:
            self.settings_window = None
        
        self.port_number = port_number
        self.rio_port = rio_port
...
#            self.file_output2.close()
    ...
    def openRioTCPConnection(self):
        start_time = time.perf_counter()
        if self.rio_port != 0:
            try:
                time_before = time.perf_counter()
                self.rio_client = AsyncSocketComms.AsyncSocketClient(self.rio_port)
                self.last_rio_update = float("-inf")
                #self.setpoint_change = 0.
                
                self.logger.info('Red_Pitaya_GUI{}: Starting rio control on port {}'.format(self.logger_name, self.rio_port))
            except Exception as e:
                print(e)
                time_after = time.perf_counter()
                #print('openTCPConnection(): Time taken by AsyncSocketComms.AsyncSocketClient(): %f sec' % (time_after-time_before))
    ...
    def closeRioTCPConnection(self):
        if self.rio_client is not None:
            try: #in case it was already close
                self.rio_client.close()
            except Exception as e:
                print('Error while closing the client: {}'.format(e))
            self.rio_client = None
            self.logger.info('Red_Pitaya_GUI{}: Closing rio control'.format(self.logger_name, self.rio_port))
...
#        self.time_history = np.zeros(self.N_history)
    ...
    def initUI(self):
        ...
        if self.output_number == 1: 
            ...
            self.qchk_clear_temp_control.setChecked(False)
            self.qchk_clear_rio_control = Qt.QCheckBox('Clear RIO feedback')
...
#        print('timerEvent, timerID = %d' % self.timerID)
    ...
    def qchk_rio_feedback_clicked(self):
    ...
    def displayFreqCounter(self):
        ...
        try:
            ...
            if DAC1_output is not None:
                ...
                if self.output_number == 1:
                    self.checkAutoUnlock(self.output_number, DAC1_output)
                    self.runTempControlLoop(time.perf_counter(), DAC1_output)
