...
class LoopFiltersUI(Qt.QWidget):
    ...
    def __init__(self, sl, filter_number=0, pll=0, bDisplayLockChkBox=True):
...
#            self.qedit_fdf.setStyleSheet("color: white; background-color: %s" % Qt.QColor(QtCore.Qt.red).name())
    ...
    def getLimits(self):
        ...
        (kd_min, kd_max) = self.pll.get_d_limits()
        self.kd_min = kd_min
        self.kd_max = kd_max
        (kdf_min, kdf_max, kfd_den) = self.pll.get_df_limits()
...
#        traceback.print_stack()
        ...
        self.pll.set_pll_settings(self.sl, P_gain, I_gain, II_gain, D_gain, D_coef, bLock)
