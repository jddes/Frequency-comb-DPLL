...
#                print 'k = %d, self.sl.DACs_limit_low[k] = %d, self.sl.DACs_limit_high[k] = %d' % (k, self.sl.DACs_limit_low[k], self.sl.DACs_limit_high[k])
                counts_offset = int(self.sl.DACs_limit_low[k] + float(self.sl.DACs_limit_high[k] - self.sl.DACs_limit_low[k]) * float(self.q_dac_offset[k].value())/1e6)
...
#                print('DAC gain in V/Counts = %f' % self.sl.getDACGainInVoltsPerCounts(k))
                ...
                VCO_gain_in_counts_per_counts = VCO_gain_in_Hz_per_Volts * self.sl.getFreqDiscriminatorGain() * self.sl.getDACGainInVoltsPerCounts(k)
...
#        print('refreshChk_event()')
        if self.qchk_refresh.isChecked() or True:
...
#        
#        self.qddc0_error_scale = Qwt.QwtThermo()
...
#        self.move(qr.topLeft())
    ...
    def initSL(self, bTriggerEvents = True):
#        self.sl = SuperLaserLand_JD2()
#        self.sl.open()
        print("initSL()")

        self.loadParameters(bTriggerEvents)
...
#            event.ignore()
    ...
    def timerDitherEvent(self):
        ...
        if not hasattr(self, 'sl'):
            print('SL object does not exist anymore. disabling timer')
        ...
        for k in range(3):
            if self.output_controls[k]:
                if self.sl.dither_enable[k] == False:
                    self.qlabel_detected_vco_gain[k].setText('---')
                    self.qlabel_detected_vco_gain[k].setStyleSheet('')
...
#               #     if self.bVeryFirstTime:
    ...
    def displayDAC(self):
        ...
        # Check if another function is currently using the DDR2 logger:
        if self.sl.bDDR2InUse:
            print('displayDAC(): DDR2 logger in use, cannot get data from adc')
            return
        # Block access to the DDR2 Logger to any other function until we are done:
        self.sl.bDDR2InUse = True        
